...
## 2026-07-20 - v0.7.0: the illustrated field guide (pixel art wave)
- Generated a cohesive pixel-art layer with PixelLab (Tier 1, ~200 generations) + Python compositing, all on the book's dark navy #020617 canvas with amber #f59e0b mono labels so it matches the existing SVG figures and the part-divider pixel art:
  - 8 hardware object sprites (photobioreactor, vertical grow-tower, LoRa mesh node, solar+battery, 3D printer, CNC plasma table, walk-behind tractor, cyberdeck), composed into a "THE FIELD KIT" specimen board (ch17-field-kit.png)
  - 4 cast character sprites (Elijah w/ notebook, Marta in welding apron, Priya w/ broadfork, Denny in beanie/hoodie), composed into a "THE CO-OP" cast plate (coop-cast.png)
  - solo plates: ch17-photobioreactor.png (SECTOR 07), appe-cyberdeck.png (Appendix E), intro-ox-plow.png ("YOU DO NOT PROGRAM AN OX", the husbandry passage)
  - 2 new house-style SVG diagrams: ch09-hyperlocal-vs-global.svg (long fragile global chain in rust vs short hyper-local loop in amber, with a repair/reuse loop-back) and ch17-algae-loop.svg (the closed fish-food cycle; photobioreactor highlighted as the input-closing node)
- Placed 7 figures with captions: intro (ox), ch9 (supply lines, under Deglobalization-Not-Decentralization), ch17 (field kit at the hardware intro; photobioreactor + algae loop in "The Proof Is Not Hypothetical"), Appendix E (cyberdeck), Part II divider (co-op cast). All in the standard figure format (image + italic caption).
- book.json 0.6.2 -> 0.7.0; compiled 85,127 words; render probe CLEAN (66 imgs resolve, zero wreckage); EPUB/PDF rebuilt at v0.7.0 (260 pages, stamped). Raw sprites + compositing scripts kept out of the repo (art-raw/ gitignored).
- NOT yet deployed: this is a large visual change to a live product, so the PDF is being shown to CT for a yes before any prod merge / website update + PDF swap.

## 2026-07-21 - v0.7.1: the wake-up-call turn (thesis + full illustration)

Two sessions, one release, integrated on weave-and-mobile-polish: editorial + thesis + conceptual diagrams + the seminal hero plate (Opus), plus a second wave of chapter pixel art and minor cast (Sonnet).

- Vibe reframe (CT direction): the book is a wake-up call and an empowerment, not a prepper / off-grid survival guide. About 20 survivalist-vibe uses of "independent" reframed to hyper-local / neighborhood / community-based (load-bearing ones kept: 1776 independence, Energy Independence, Independent Seed Bank, game-theory actors). "Grid independence/insulation" reframed to emergency resilience ("keeps functioning without the global grid in an emergency"; the test is "can your community survive if the trucks stop?"), not permanent secession.
- Thesis: the seminal question added to the Introduction ("if a robot can grow my food and build my house, why am I still renting my survival?") with a Conclusion callback; Ch9 gains the dense-city-vs-surrounding-municipalities passage (a dense core cannot produce everything in-limits, the ring of surrounding municipalities can, which is a jobs program at Manhattan-Project scale; resilience, not secession).
- Credibility: the two "300% metabolic yield" overclaims in Ch12 grounded to DIY reality (a garden will not usually cover full caloric needs; automation makes a surplus possible; the bottleneck was distribution, not productivity). Johnny Autoseed removed from the book entirely (Ch9 bullet + Agriculture table + Appendix B citations, which became verified Open Source Ecology + Mycodo) so the book stands as CT's own work, not a company promo.
- Interactive removed: the Local Grid Sizer explorable (Ch17) pulled at CT's call (off-message prepper framing); the generic interactive framework left intact but empty.
- Art wave 1 (house-style SVG diagrams + PixelLab): the nine-stages master ladder (Intro, Stage 4 "The New Social Contract" highlighted as the hinge), the old-vs-new social contract (Ch18, beside ALC), the region-ring (Ch9). Plus a PixelLab hero plate for the Introduction, "THE ROBOTS THAT GROW YOUR FOOD AND BUILD YOUR HOUSE" (a grower-bot and a builder-bot, composited on the navy/amber canvas via the art-raw Pillow pipeline).
- Art wave 2 (chapter illustration): 9 new pixel-art object plates pulled from each chapter's own prose (Ch1 GPU rig, Ch5 wood stove, Ch6 tool wall, Ch9 controller, Ch11 Crucible rack, Ch13 bench vise, Ch14 bell, Ch15 soil probe, conclusion printed proofs) and 4 previously-unillustrated named cast (Reuben, Frank, Gary, Devendra), same field-guide style, placed inline at the right passage.
