...
WORK = ROOT / 'book-build/v0.8.1-source'
PROOF = ROOT / 'book-build/v0.8.1-proof'
PROOF.mkdir(exist_ok=True)
SOURCE = ROOT / 'src/lib/data/book'
meta = json.loads((SOURCE / 'book.json').read_text())
reader = PdfReader(ROOT / 'book-build/Surviving-the-Singularity-v0.8.1.pdf')
...
needles = ['What if it all goes right?', 'Doesn’t change your dinner', 'Dinner’s dinner',
           'beyond June', 'funded through June', 'Some could close', 'v0.8.1',
...
clean_text = re.sub(r'Surviving the Singularity\s*·\s*v0\.8\.1\s+\d+', '', text)
