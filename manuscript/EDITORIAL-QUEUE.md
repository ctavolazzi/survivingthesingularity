...
**LATEST VERSION: v0.7.5 (2026-08-02).** v0.7.5 = the reading-rage plate. One figure into Precedent P-01 in the preface (`preface-reading-rage.png`): a young woman reading a novel by candlelight in a 1790s parlour, the object of the panic rather than the panickers, because P-01's argument is that the panic located where the power had moved. PixelLab sprite (`sts.sprite.novel-reader`) composited by the new `scripts/build_book_plate.py`, which is the first time the figure-plate layout has existed anywhere but inside the output PNGs — it rebuilds `ch01-pc-rig` and `ch08-gary` from their sprites and diffs against the committed files (0.578% / 0.366% of pixels, antialiasing only), and it fails loudly when a constant is wrong. 90,647 words, 30 sections, 23 precedents, 86 figures linked, `sts.py verify` clean, `id verify` clean. NO release artifact built: the downloadable PDF/EPUB is still v0.7.4 and `RELEASES.md` says so. NOT merged to main. --- v0.7.4 = the canned music build: Precedent P-10 ("The Robot in the Orchestra Pit," 1929-1948) in Chapter 7 plus the two period AFM advertisements, the first primary-source documents printed as figures; KaTeX math pass on ch4 and ch8; built at 289 pages / 90,596 words. --- v0.7.3 = the seventeen-definitions pass and the P-19 page-count correction; never built as its own artifact, shipped inside v0.7.4. --- v0.7.1 = the wake-up-call turn. Vibe reframe (not a prepper/off-grid guide, a wake-up call): ~20 survivalist-vibe uses of "independent" moved to hyper-local/neighborhood/community-based (load-bearing ones kept), grid independence reframed to emergency resilience. Thesis: the seminal question in the Intro ("if a robot can grow my food and build my house, why am I still renting my survival?") + Conclusion callback; Ch9 dense-city/surrounding-municipalities passage. Credibility: 300% overclaims grounded, Johnny Autoseed removed everywhere (Appendix B -> Open Source Ecology + Mycodo). Local Grid Sizer interactive removed. Illustration: 3 house-style SVG diagrams (nine-stages, social-contract, region-ring) + a PixelLab robots hero plate, plus a 2nd wave of 9 chapter object plates + 4 cast (Reuben/Frank/Gary/Devendra). 83/83 figures linked, id verify clean, EPUB/PDF at 275 pages. Public PDF: /downloads/Surviving-the-Singularity-v0.7.1.pdf. NOT merged to main (awaiting CT's explicit yes). --- v0.6.2 = two word rulings on top of v0.6.1: "manifesto" removed everywhere (book prose ch1/ch5, internal register renamed "Declaration"; only Bastani's real citation subtitle survives in Appendix B) and decentralization de-heralded in favor of hyper-local/independent/deglobalization (kept only where it names actual decentralized tech: mesh, LoRa, IPFS, crypto, distributed energy; anchor thesis already in ch9). Public PDF: /downloads/Surviving-the-Singularity-v0.6.2.pdf. --- v0.6.1 = the techno-optimist
