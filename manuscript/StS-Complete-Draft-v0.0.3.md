...
**Complete Compiled Draft v0.0.3 — 2026-07-10**

*Recompiled from `src/lib/data/book/` (the canonical chapter source driving the live site and EPUB build) after merging the Draft 5 / “Manual for the Cracks” practical-mechanics prose into the Part II–III stub chapters. Terminology audit (Sovereign → Autonomy) applied to all merged material. See Compilation Notes at the end.*
...
The robots have taken over. Congratulations! You've lost.

But losing the old world is the exact tactical opening you need to become aware of the one that's already here.

For millennia, humanity has been the smartest critter on the block. That is about to change forever.

The corporate state bureaucrats want to fence this moment behind a digital moat, metering your survival while trapping you in a system of debt and surveillance. They think they can put a legal collar on a machine god.
...
This moment presents our first real opportunity to stop hiding behind fear and insecurity. We already possess the technology to solve the problems that have plagued our species since the very beginning, yet the powers that be choose doubt at every turn.

Will the machines make the same choice?
...
The endgame isn't just making enough money to buy a bunker until this all blows over. We can build new systems that secure physical survival on the local level, strip away the bullshit, and guarantee our children the future they deserve.
...
*This section is editorial metadata, not book content. Recompiled 2026-07-10.*

**This draft supersedes v0.0.1** (and the unreleased v0.0.2 recompile from earlier today).
Two things happened in this pass:

1. **Source-of-truth recompile.** The manuscript is reassembled directly from
   `src/lib/data/book/*.md` — the single canonical chapter source that also feeds the live
   `/book` site and `scripts/build-epub.sh` — closing the drift that had opened between the
   v0.0.1 manuscript file (2026-07-01) and later chapter edits (Ch. 5, 8, 14–17).

2. **Draft 5 merge.** The "STS-Draft5" Google Doc (2026-05-30, the *Manual for the Cracks*
   lineage, ~23.8k words) was recovered in a full local + Drive source sweep
   (`manuscript/SOURCE-MANIFEST.md`) and its practical-mechanics chapters were merged
   verbatim into the stub chapters:

   | Draft 5 chapter | Merged into |
   |---|---|
   | Ch. 5 The Sovereign Shell ($25k Exit) | Ch. 13 The Shouse Protocol |
   | Ch. 7 The Crucible (local servers) | Ch. 11 Using the Tech to Your Advantage |
   | Ch. 11 The Sovereign Horizon (CSA/Shouse Grid) | Ch. 12 The Land Strategy |
   | Ch. 8 The Robot in the Soil (FarmBot) | Ch. 9 Deglobalization & the Neighborhood Factory |
   | Ch. 4 The Great Slop-ocalypse & Algorithmic Judo | Ch. 10 The "Create Over Consume" Protocol |
   | Ch. 3 Cognitive Firewalls & Semantic Hygiene | Ch. 8 The Psychology of the Collapse |
   | Ch. 9 Philosophy of the Commons (Relational Sovereignty) | Ch. 8 — as **Relational Autonomy** section |

   Draft 5 chapters 1 (Thermodynamics), 2 (Long Tail), 6 (DC-Native Solar), and 10 (Mesh
   Networks) were **not** merged: their material already exists in healthy chapters 5, 14,
   and 17.

   Mechanical passes applied to all merged prose: Google-Docs export cleanup (LaTeX
   artifacts converted to plain notation, ASCII diagrams and code listings re-fenced,
   escaped characters unescaped) and the terminology audit (Sovereign/Sovereignty →
   Autonomy variants; "Sovereign Shell" → "Autonomous Shell"; "Relational Sovereignty" →
   "Relational Autonomy"). `grep -ri sovereign src/lib/data/book/` returns clean.

**Second fold (2026-07-10, same day):** eight source documents supplied by the author
were folded in — the *Singularity Research and Evidence Synthesis* body sections
(Simulation Trilemma → Ch. 4; Economic Paradox/FALC → Ch. 6; Psychological Bottleneck
(Jahoda/Keynes/scarcity mindset) + empirical Primate Backlash (Safe Street Rebels/Waymo)
→ Ch. 7; Shouse Grid/Brooklyn Microgrid P2P energy → Ch. 9; Synthesis & New Social
Contract (Abundance Quotient) → Ch. 18), *The Empowerment Architecture* (Mathematical
Dead End → Ch. 6; two-audience beacon + Cyber-Deck curriculum → Ch. 18; Moral Mandate →
Conclusion), and the *9 Stages Executive Reference Guide* as new **Appendix C**.
The Page-1 Introduction ("The Uncompromising Truth") is preserved exactly as written
in the source document — verified verbatim, untouched by every pass.

**Still open (see `manuscript/EDITORIAL-QUEUE.md`):**

- Ch. 3 (Leap to ASI, 872w) remains the only thin chapter.
- "Egalitarian Pivot" outline section still has no drafted prose.
- Stage-2 real-world evidence weave for Ch. 2.
- Voice-unification pass (merged material spans registers).

**Total word count this pass:** 39,577 words (incl. front/back matter, excl. back
cover copy and these notes). Target per EDITORIAL-QUEUE.md: 45,000–50,000.
