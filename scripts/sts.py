...
# Routes that must never appear in sitemap.xml because they are noindex.
# Everything under /book and /read sits behind the book password gate, so the
# only thing a crawler can reach there is the gate itself; /exclusive-friends-only
# is deliberately unlisted. Listing a noindex route in the sitemap is a straight
# contradiction - it invites the crawl and then refuses the index - and /book
# was doing exactly that.
NOINDEX_PREFIXES = ("/book", "/read", "/exclusive-friends-only")


def is_noindex(route: str) -> bool:
    return any(route == p or route.startswith(p + "/") for p in NOINDEX_PREFIXES)


def public_pages(pages: list) -> list:
    """Pages that belong in the sitemap (skip utility/dynamic/noindex)."""
    skip = {"/unsubscribe", "/early-access/success"}
    return [p for p in pages
            if "[" not in p and p not in skip and not is_noindex(p)]
...
def check_sitemap(pages: list):
    ...
    for ghost in sorted(listed - real):
        if is_noindex(ghost):
            errors.append(f"sitemap.xml lists {ghost}, which is noindex — the sitemap "
                          "invites the crawl and the page then refuses the index")
        else:
...
def og_declared() -> dict:
    """{route: og:image content} for every page that declares one."""
    found = {}
    for marker in ROUTES_DIR.rglob("+page.svelte"):
        rel = marker.parent.relative_to(ROUTES_DIR)
        route = "/" + "/".join(rel.parts) if rel.parts else "/"
        ...
        m = re.search(r'property="og:image"\s+content=(?:"([^"]*)"|\{([^}]*)\})',
                      head.group(1))
        if m:
            # content={post.image} is a real declaration whose value only exists
            # at runtime; record it as dynamic rather than calling it missing.
            found[route] = m.group(1) if m.group(1) is not None else f"{{{m.group(2)}}}"
    return found
