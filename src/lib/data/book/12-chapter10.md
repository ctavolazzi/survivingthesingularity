...
Let's express this iterative training process mathematically. Let the real human data distribution be $P_0(x)$. A model $M_k$ at generation $k$ is trained on data $X_k$ sampled from the distribution $P_{k-1}(x)$ generated by the previous model $M_{k-1}$.
