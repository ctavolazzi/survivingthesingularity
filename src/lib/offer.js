...
/**
 * TWO COUNTS. THEY COUNT DIFFERENT OBJECTS. DO NOT COLLAPSE THEM.
 *
 * This is the field that has been wrong the most times in this project, and
 * every time it was wrong because one number was being asked to describe two
 * artifacts.
 *
 ...
 *
 * `PRECEDENT_FILE_CASE_COUNT` is the BONUS FILE's cases, and it is measured. The
 * research turned up more cases than the book had chapters to close, so this
 * number is legitimately larger. scripts/build_bonus.py counts the `### A1.`
 * style case headings in the document it just emitted and writes the result into
 * bundleManifest.js, which is why this line reads from a manifest instead of
 * holding an integer.
 *
 * WHAT WENT WRONG BEFORE, so nobody re-derives the same bug:
 *   - The live site said 29 on four surfaces until 2026-07-29. Nothing was ever
 *     counted to produce 29.
 *   - The fix set this constant to 23, which corrected the site but silently
 *     applied the BOOK's ledger count to the BONUS FILE. The confirmation email
 *     a paying customer received then described a 31-case file as having 23.
 *   - build_bonus.py separately printed "cases indexed: 28", which is neither:
 *     it counts headings carrying a Sources line.
 *
 * Four numbers, one artifact, none of them measured. That is what this split
 * closes. C-02, done.
 */
/**
 * RELATIVE PATH, DELIBERATELY, and not the `$lib` alias every other file here
 * uses. scripts/check-offer-drift.mjs imports this module in PLAIN NODE, not
 * through Vite, so it gets no alias resolution and no JSON import support. A
 * `$lib` specifier would throw ERR_MODULE_NOT_FOUND and take the whole prebuild
 * gate down with it; a `.json` import would need an import attribute. A relative
 * `.js` import is the one form that works in both runtimes.
 *
 * Generated by scripts/build_bonus.py from the bundle it just built. If it is
 * missing, run `python3 scripts/sts.py bundle build`.
 */
...
export const offer = Object.freeze({
  ...
  included: Object.freeze([
    ...
    /**
     * The sourced count is stated rather than rounded up to "every source
     * cited", which is what this line used to say and which the measurement
     * does not support: three of the cases carry no sources yet, and the file
     * names them. On a product whose entire pitch is "check the work", the
     * honest fraction is worth more than the tidy claim, and it is derived so
     * it cannot quietly become the tidy claim again.
     */
    Object.freeze({
      id: 'precedent-file',
      title: 'The Precedent File.',
      detail:
        `${PRECEDENT_FILE_CASE_COUNT} documented cases of people meeting a ` +
        'machine that changed everything, ' +
        `${PRECEDENT_FILE_SOURCED_COUNT} of them with their sources listed ` +
        'inline so you can check the work.',
      available: true
    }),
    Object.freeze({
